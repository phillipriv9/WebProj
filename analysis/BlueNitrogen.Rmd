...
model <- lmer(N_perc ~  OC_perc*Habitat + centerdepth + (1 | study_id) + (1 | core_id), data = df3, REML = FALSE)

#This null model is looking at the interaction of Oxygen concentration percent with Habitat type.
model.interaction.null <- lmer(N_perc ~ OC_perc+Habitat + centerdepth + (1 | study_id) + (1 | core_id), data = df3, REML = FALSE)

#Null model looking at center depth
model.centerdepth.null <- lmer(N_perc ~ OC_perc*Habitat + (1 | study_id) + (1 | core_id), data=df3, REML = FALSE)

model.centerdepth.null
model
model.interaction.null
#Perfoming likelhood tests to determined the significance of our fixed effects and interactions
